    ...
    for (const email of emails) {
      ...
      try {
        ...
        if (ticketNumber) {
          ...
          // Look up assignment group based on mailbox email
          // This ensures tickets sent to specific mailboxes are routed to the correct queue
          let assignmentGroupId: string | null = null;
          const { data: groupData } = await supabase
            .from('operator_groups')
            .select('id')
            .or(`group_key.eq.servicedesk-queue,name.ilike.%service desk%`)
            .eq('is_active', true)
            .limit(1)
            .single();
          
          if (groupData) {
            assignmentGroupId = groupData.id;
          }

          // Create ticket - ticket_number is auto-generated by database trigger
