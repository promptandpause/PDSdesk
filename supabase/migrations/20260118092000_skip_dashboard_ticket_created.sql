-- Skip ticket creation emails for dashboard users (channel = 'dashboard')
-- Keep sending for internal staff (other channels)
...
BEGIN
  ...
  -- Skip ticket creation emails for dashboard users
  IF p_event_type = 'created' AND v_ticket.channel = 'dashboard' THEN
    RETURN NULL;
  END IF;
  
  -- Use requester_email field if available (for tickets created by system user on behalf of customer)
  -- Otherwise fall back to the profile email
  v_requester_email := COALESCE(
    p_recipient_email, 
    v_ticket.requester_email,  -- Direct field from tickets table
    v_ticket.email            -- From profiles join
  );
  ...
  -- For updated tickets, get the latest comment to include in email
  IF p_event_type = 'updated' THEN
    SELECT body INTO v_latest_comment
