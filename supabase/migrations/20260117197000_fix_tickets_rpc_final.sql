-- Fix get_all_tickets_for_admins - use correct column names from tickets table
-- assignee_id (not assigned_to), assignment_group_id (correct)
...
begin
  ...
  where (p_status is null or t.status::text = p_status)
    and (p_queue_id is null or t.assignment_group_id = p_queue_id)
